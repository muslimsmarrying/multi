...
const updateAdminController = async (req, res) => {
  try {
    ...
    const updateData = {
      ...
      batchPayment: batchPayment || existingAdmin.batchPayment,
      batchAllowedUsers: batchAllowedUsers || existingAdmin.batchAllowedUsers,
    };
...
const getAllActivities = async (req, res) => {
  try {
    const adminActivities = await AdminActivity.find()
      .populate("adminId", "name email") // Populate admin basic info
      .sort({ createdAt: -1 });
...
const getSingleAdminDetails = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const admin = await adminModel
      .findById(id)
      .select(
        "-password -createdAt -lastLogin -totpSecret -updatedAt -resetPasswordToken -resetPasswordExpiresAt -verificationToken -verificationTokenExpiresAt"
      );
    ...
    console.error("Error in getSingleAdminDetails:", error.message);
    res.status(500).json({
      success: false,
      message: "Server error",
      error: error.message,
...
const sendLoginOtp = async (email, name, verificationToken) => {
  ...
  const mailOptions = {
    ...
    to: email,
    subject: "Login otp to Cure sleep solutions",
    html: `<div style="background: #121212 ; text-align: center;">
       <h1 style="font-size: 45px; font-weight: bold; background: #121212; color:#fff;padding:10px 0px; " >Cure sleep solutions</h1>
    </div>
    <div style="background-color: #f9f9f9; padding: 20px; border-radius: 0 0 5px 5px; box-shadow: 0 2px 5px rgba(0,0,0,0.1);">
      ...
      <h5 style="color: #333; font-size: 17px; margin-bottom: 5px;">Best regards,<br>Cure sleep solutions Team</h5>
    </div>
...
const sendLoginEmail = async (email, name, time, date) => {
  try {
    ...
    const mailOptions = {
      from: process.env.AUTH_EMAIL_P,
      to: email,
      subject: "Cure sleep solutions Alert",
      html: `<div style="background: #121212; padding: 20px; text-align: center;">
                <h1 style="color: white; margin: 0;">Cure sleep solutions</h1>
            </div>
            <div style="background-color: #f9f9f9; padding: 20px; border-radius: 0 0 5px 5px; box-shadow: 0 2px 5px rgba(0,0,0,0.1);">
                <h5 style="font-size:20px">Dear  ${name},</h5>
                <p style="font-size:17px">You have  successfully logged in to Cure sleep solutions  at <strong>${time}</strong> on <strong>${date}</strong>.</p>
                ...
                <p style="font-size:17px">Please note that Cure sleep solutions will never ask for any confidential information by calling from any number including its official helpline numbers, through emails or websites! Please do not share your confidential details such as  CVV, User Name, Password, OTP etc.</p>
                <p>In case of any complaint, you may contact us through:</p>
                <ul>
                    <li style="font-size:17px">Email: <a href="mailto:support@Cure sleep solutions.com">support@Cure sleep solutions.com</a></li>
                    <li style="font-size:17px">Phone: <a href="tel:+442071775747">+44 2071775747</a></li>
                    <li style="font-size:17px">Websites: <a href="https://Cure sleep solutions.com/contact">https://Cure sleep solutions.com/contact</a></li>
                </ul>
            </div>
            <div style="text-align: center; margin-top: 20px; color: #888; font-size: 13px;">
...
<div style="background-color: #f9f9f9; padding: 20px; border-radius: 0 0 5px 5px; box-shadow: 0 2px 5px rgba(0,0,0,0.1);">
    <h5 style="font-size:20px">Dear  ${name}, </h5>
    <p style="font-size:17px">Cure sleep solutions recently received a request for a forgotten password.</p>
    <p style="font-size:17px">To change your Cure sleep solutions password, please click on below link</p>
    ...
    <h5 style="font-size:17px">Best regards,<br>Cure sleep solutions Team</h5>
